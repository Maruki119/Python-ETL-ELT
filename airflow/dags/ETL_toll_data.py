...
destination_dir = "/opt/airflow/dags/finalassignment"
...
with DAG(
    ...
    unzip_data = BashOperator(
        task_id="unzip_data",
        bash_command="tar -xvf " + destination_dir + "/tolldata.tgz -C " + destination_dir,
        dag=dag
    )
    ...
    extract_data_from_tsv = BashOperator(
        ...
        bash_command=f"""
            echo "Number of axles,Tollplaza id, Tollplaza code" > {destination_dir}/tsv_data.csv
            cut -f5-7 {destination_dir}/tollplaza-data.tsv \
            | tr '\\t' ',' \
            | tr -d '\\r' \
            > {destination_dir}/tsv_data.csv
        """,
        dag=dag
    )
    ...
    consolidate_data = BashOperator(
        ...
        bash_command=f"""
            paste -d',' {destination_dir}/csv_data.csv {destination_dir}/tsv_data.csv {destination_dir}/fixed_width_data.csv > {destination_dir}/extracted_data.csv
        """,
        dag=dag
    )
